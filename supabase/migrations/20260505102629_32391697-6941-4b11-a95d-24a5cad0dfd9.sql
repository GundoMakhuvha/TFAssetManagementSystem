...
-- Enums
do $$ begin create type public.app_role as enum ('admin','technician','viewer'); exception when duplicate_object then null; end $$;
do $$ begin create type public.department_t as enum ('CSS','Finance','IT','Facilities','Tipp Con'); exception when duplicate_object then null; end $$;
do $$ begin create type public.asset_condition_t as enum ('Good','Fair','Poor','Damaged'); exception when duplicate_object then null; end $$;
do $$ begin create type public.ticket_priority_t as enum ('Low','Medium','High','Critical'); exception when duplicate_object then null; end $$;
do $$ begin create type public.ticket_status_t as enum ('Open','In Progress','On Hold','Resolved','Closed'); exception when duplicate_object then null; end $$;
do $$ begin create type public.ticket_category_t as enum ('Hardware','Software','Network','Access','Other'); exception when duplicate_object then null; end $$;
do $$ begin create type public.verification_method_t as enum ('barcode','manual'); exception when duplicate_object then null; end $$;
...
create policy "tickets_read" on public.tickets for select to authenticated
  using (submitted_by = auth.uid() or public.has_role(auth.uid(),'admin') or public.has_role(auth.uid(),'technician'));
drop policy if exists "tickets_insert" on public.tickets;
create policy "tickets_insert" on public.tickets for insert to authenticated with check (submitted_by = auth.uid());
drop policy if exists "tickets_update" on public.tickets;
create policy "tickets_update" on public.tickets for update to authenticated
  using (public.has_role(auth.uid(),'admin') or public.has_role(auth.uid(),'technician') or submitted_by = auth.uid())
  with check (public.has_role(auth.uid(),'admin') or public.has_role(auth.uid(),'technician') or submitted_by = auth.uid());
...
create policy "comments_read" on public.ticket_comments for select to authenticated
  using (exists (select 1 from public.tickets t where t.id = ticket_id
    and (t.submitted_by = auth.uid() or public.has_role(auth.uid(),'admin') or public.has_role(auth.uid(),'technician'))));
drop policy if exists "comments_insert" on public.ticket_comments;
create policy "comments_insert" on public.ticket_comments for insert to authenticated with check (author_id = auth.uid());
...
create policy "ticket_att_read" on storage.objects for select to authenticated using (bucket_id = 'ticket-attachments');
drop policy if exists "ticket_att_insert" on storage.objects;
create policy "ticket_att_insert" on storage.objects for insert to authenticated with check (bucket_id = 'ticket-attachments');